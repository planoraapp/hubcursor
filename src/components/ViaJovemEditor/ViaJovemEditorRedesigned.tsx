...
export const ViaJovemEditorRedesigned = ({ className = '' }: ViaJovemEditorRedesignedProps) => {
  ...
  return (
    <div className={`max-w-7xl mx-auto p-4 space-y-6 ${className}`}>
      ...
      <div className="flex justify-center gap-2">
        <button
          className={`px-4 py-2 rounded ${selectedGender === 'M' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          onClick={() => setSelectedGender('M')}
        >
          👨 Masculino
        </button>
        <button
          className={`px-4 py-2 rounded ${selectedGender === 'F' ? 'bg-pink-500 text-white' : 'bg-gray-200'}`}
