-- ========================================
-- MIGRATION: HABBO HUB LOGIN SYSTEM
...
-- Migrar dados existentes da tabela 'users' para 'hub_users' se existir
DO $$
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users' AND table_schema = 'public') THEN
    ...
    SELECT 
      COALESCE(habbo_username, 'unknown'),
      COALESCE(hotel, 'br'),
      COALESCE(habbo_avatar, ''),
      COALESCE(password_hash, ''),
      COALESCE(member_since, ''),
      COALESCE(created_at, NOW()),
      COALESCE(is_verified, TRUE)
    FROM public.users
    WHERE habbo_username IS NOT NULL
    ON CONFLICT (habbo_username, hotel) DO NOTHING;
    
    -- Log da migração
    RAISE NOTICE 'Dados migrados da tabela users para hub_users';
  END IF;
END $$;
